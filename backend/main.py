...
def _extract_shap_values(shap_values: Any, class_idx: int) -> np.ndarray:
...
def _explain_prediction(vector: np.ndarray, predicted_disease: str, top_n: int = 10) -> dict[str, Any]:
    ...
    try:
        ...
        class_shap = _extract_shap_values(shap_values, class_idx)
        pairs = [
            {
...
def health() -> dict[str, Any]:
    return {
        ...
        "total_diseases": int(len(getattr(MODEL, "classes_", []))),
    }
...
def predict(payload: PredictionRequest) -> dict[str, Any]:
    ...
    prediction, top_predictions, confidence = _prediction_top_k(vector, k=5)
    risk_level = _risk_level(confidence)
    explanation = _explain_prediction(vector, prediction, top_n=10)
